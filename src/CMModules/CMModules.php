<?php
/**
 * A model for managing Mad modules.
 * 
 * @package MadCore
 */
class CMModules extends CObject {
   ...
   * Properties
   */
  private $madCoreModules = array('CMad', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject');
  private $madCMFModules = array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier');
  ...
  private function GetDetailsOfModule($module) {
    ...
    if(class_exists($module)) {
      ...
      $details['isMadCore']   = in_array($rc->name, $this->madCoreModules);
      $details['isMadCMF']    = in_array($rc->name, $this->madCMFModules);
